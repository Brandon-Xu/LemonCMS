...
<div class="mainbody">
    ...
    <div class="inst_btn_area">
        ...
        <form name="form" method="post" action="<?php echo Url::to(['db']) ?>">
            <button type="button" onclick="window.location='<?php echo Url::to(['index']) ?>'" class="button">返回
            </button>
            <?php if ($requireResult == 0 || $writeableResult == 0): ?>
                <button type="button" class="button disable" disabled="disabled">环境检测未通过，请根据提示修正后再尝试安装</button>
            <?php else: ?>
                <button type="button" onclick="window.location='<?php echo Url::to(['db']) ?>'" class="button">下一步
                </button>
            <?php endif ?>
            &nbsp;
        </form>
