...
class Generator extends \yii\gii\Generator
{
    ...
    public function attributeLabels() {
        return [
            'moduleID' => '模块ID', 'moduleDir' => '模块目录名称',
        ];
    }
    ...
    public function hints() {
        return [
            'moduleID' => '模块的ID, e.g., <code>admin</code>.', 'moduleDir' => '文件夹名称，如<code>post,shop,news</code>',
        ];
    }
    ...
    public function successMessage() {
        if (Yii::$app->hasModule($this->moduleID)) {
            $link = Html::a('try it now', Yii::$app->getUrlManager()
                ->createUrl($this->moduleID), ['target' => '_blank']);
...
EOD;
    ...
    private function generateInfo($modulePath) {
        $files[] = new CodeFile($modulePath.'/'.$this->getModuleClassName().'Info.php', $this->render("module-info.php"));

        return $files;
    }

    private function generateService($modulePath) {
        $files[] = new CodeFile($modulePath.'/'.$this->getModuleClassName().'Service.php', $this->render("service.php"));

        return $files;
    }

    private function generateAdmin($modulePath) {
        $files[] = new CodeFile($modulePath.'/admin/AdminModule.php', $this->render("admin-module.php"));
        $files[] = new CodeFile($modulePath.'/admin/controllers/DefaultController.php', $this->render("admin-controller.php"));
        $files[] = new CodeFile($modulePath.'/admin/views/default/index.php', $this->render("admin-view.php"));

        return $files;
    }

    private function generateHome($modulePath) {
        $files[] = new CodeFile($modulePath.'/home/HomeModule.php', $this->render("home-module.php"));
        $files[] = new CodeFile($modulePath.'/home/controllers/DefaultController.php', $this->render("home-controller.php"));
        $files[] = new CodeFile($modulePath.'/home/views/default/index.php', $this->render("home-view.php"));

        return $files;
    }

    private function generateSetting($modulePath) {
        $files[] = new CodeFile($modulePath.'/models/Setting.php', $this->render("setting-model.php"));
        $files[] = new CodeFile($modulePath.'/admin/Controllers/SettingController.php', $this->render("setting-controller.php"));
        $files[] = new CodeFile($modulePath.'/admin/views/setting/index.php', $this->render("setting-view.php"));
